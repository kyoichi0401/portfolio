class Mypage::UsersController < ApplicationController
  ...
  def edit
    ...
    if @conference == nil
      ...
      @slots = @conference.days.map{|d| d.tracks.map{|t| t.slots }}.flatten.sort_by{|s| [s.start_time, s.track.seq_no] }
    end
  ...
  def update
    ...
    if p != nil
      ...
      p.each{|sid| Participation.create(user: u, slot: Slot.find(sid), conference: c)
      }
